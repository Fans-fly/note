...
* Map(namespace, Map(group::serviceName, Service)).
  */
  ...
  服务健康检查：Nacos server开定时任务检查服务实例的健康情况，超过15秒没收到心跳的设置healthy属性为false（客户端服务发现时不会发现），如果某个实例30秒没收到心跳，直接踢出（如果之后又恢复了，再重新注册），删除调instance的delete接口。
...
Q4：如果服务实例有变动，比如新服务注册，客户端如何近实时感知？已知是有默认5s拉一次服务列表的。---在新服务注册完成时，会发布一个事件：com.alibaba.nacos.naming.push.PushService#serviceChanged，发布事件：ServiceChangeEvent。如何找到此事件处理的逻辑？--全局搜这个事件，com.alibaba.nacos.naming.push.PushService#onApplicationEvent。最终使用UDP协议(使用udp协议易丢失消息，但可以忽略，因为就算发送失败，客户端5s定时任务也会重新拉取新服务列表)，服务端发送事件变化到客户端：com.alibaba.nacos.naming.push.PushService#udpPush-->java.net.DatagramSocket#send。UDP协议具体使用哪个端口通信？服务发现时，服务端会将UDP通信端口发送到客户端。
...
1、服务发现源码入口：服务发现其实是根据第一次调用服务时根据服务名去服务端获取的，ribbon源码解释。入口方法名：NacosNamingService.getAllInstances();

2、先查询客户端服务缓存信息，因为是首次调用，为空；此时com.alibaba.nacos.client.naming.core.HostReactor#updateServiceNow调用server接口
...
官方集群版部署文档参考--使用部署包：https://nacos.io/zh-cn/docs/cluster-mode-quick-start.html

源码版集群启动方式：

> 1、创建数据库，库名随意；然后执行 distribution/conf 目录下的 nacos-mysql.sql 脚本，然后修改 console\src\main\resources 目录下的 application.properties 文件里的mysql配置。
>
> 2、运行console模块里的 com.alibaba.nacos.Nacos.java，需要增加启动vm参数端口号和实例运行路径nacos.home(对应的目录需要自
>
> 己提前创建好)，每台server的nacos.home目录里需要创建一个conf文件夹，里面放一个cluster.conf文件，文件里需要把所有集群机
>
> 器ip和端口写入进去；conf内容和启动参数参考下图：-Dserver.port=8847 -Dnacos.home=D:\nacos-cluster\nacos-8847

![image.png](./assets/1668152319503-image.png)

![image.png](./assets/1668152374820-image.png)

![image.png](./assets/1668152389506-image.png)

![image.png](./assets/1668152394460-image.png)

#### 6.1 Nacos心跳在集群架构下的设计原理剖析

> 心跳任务放在其中一个节点上，加入有服务变动，新增、删除、更新；由心跳任务节点将变动通知到集群的其他节点。
>
> 如何选择心跳任务放在哪台主机呢？--根据serviceName做hash，然后对集群节点数量取模

#### 6.2 Nacos集群节点状态同步源码剖析

#### 6.3 Nacos集群服务新增数据同步源码剖析

#### 6.4 Nacos集群服务状态变动同步源码剖析
